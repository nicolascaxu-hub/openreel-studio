...
def reset_confirmation_text() -> str:
    return (
        "全量重置需要确认。该操作会清空任务、面板、画布节点、连边和项目内的人物、"
...
def reset_success_text(result: dict[str, Any], scope: str) -> str:
    ...
    if scope == "full":
        ...
        if applied:
            return f"项目已重置。画布已清空（{deleted_count} 个节点）。新主题已应用：{json.dumps(applied, ensure_ascii=False)}"
        return f"项目已重置。画布已清空（{deleted_count} 个节点），标题已恢复为「未命名项目」。请告诉我新项目想做什么？"
